...
app.config['SQLALCHEMY_DATABASE_URI']='mysql://root:@localhost/db_laika'
db=SQLAlchemy(app)
...
def  pantalones():
    query1 = text("""
        SELECT clientes.id_cliente, clientes.nombre_cliente
        FROM clientes
        WHERE clientes.nombre_categoria = 'Pantalones'
    """)
    query2 = text("""
        SELECT productos.id_producto, productos.descripcion, productos.precio
        FROM productos
        WHERE productos.nombre_categoria = 'Pantalones'
    """)
    query3 = text("""
        SELECT empleados.id_empleado, empleados.nombre_empleado
        FROM empleados
        WHERE empleados.nombre_categoria = 'Pantalones'
    """)
    ...
    return render_template("pantalones.html", cliente=cliente, producto=producto, empleado=empleado)
...
def  camisetas():
    query1 = text("""
        SELECT clientes.id_cliente, clientes.nombre_cliente
        FROM clientes
        WHERE clientes.nombre_categoria = 'Camisetas'
    """)
    query2 = text("""
        SELECT productos.id_producto, productos.descripcion, productos.precio
        FROM productos
        WHERE productos.nombre_categoria = 'Camisetas'
    """)
    query3 = text("""
        SELECT empleados.id_empleado, empleados.nombre_empleado
        FROM empleados
        WHERE empleados.nombre_categoria = 'Camisetas'
    """)
    ...
    try:
        ...
        producto = []  
        empleado = []
    return render_template("camisetas.html", cliente=cliente, producto=producto, empleado=empleado)
